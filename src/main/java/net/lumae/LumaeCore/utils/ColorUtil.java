...
public class ColorUtil {
	...
	private static final SortedMap<String, String> colorMap = null;
	private static final String HEX_REGEX = "#[A-Fa-f0-9]{6}";
	private static final String TAG_REGEX = "^[A-Za-z0-9_]{1,16}$";
	private static final String RAW_HEX_REGEX = "<#[A-Fa-f0-9]{6}>";
	private static final String RAW_GRADIENT_HEX_REGEX = "<\\$#[A-Fa-f0-9]{6}>";
	private static String format = "<@{TAG}>";
	...
	public static String colorMessage(Player player, String legacyMsg, boolean checkPermissions) {
		...
		for (String code : colorMap.keySet()) {
			String rawCode = format.replace("{TAG}", code);
			...
			legacyMsg = legacyMsg.replaceAll(rawCode, ChatColor.of(colorMap.get(code)) + "");
		}
	...
	private static String gradient(String legacyMsg) {
		for (String code : colorMap.keySet()) {
			...
			if (!legacyMsg.toLowerCase().contains(rawCode)) continue;
			legacyMsg = legacyMsg.replaceAll("\\$" + rawCode, "\\$" + colorMap.get(code));
		}
